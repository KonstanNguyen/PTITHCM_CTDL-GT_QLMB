...
void InputFlight(PNodeFli &first, Flight &flight, ListAir listAir, bool Edit = false, bool Cancle = false)
{
	...
	while (true)
	{
		...
		{ // Nhap DateTime
			do
			{
				Clean(X_Add + 13, Y_Add + 9, X_Add + 24, Y_Add + 9);
...
void ChangeFlightMenuManagerPage(PNodeFli first)
{
	DisplayTest(xKeyDisplayFli, ContentFlight, sizeof(ContentFlight) / sizeof(std::string));
...
void ChangeAirplaneFlyMenuManagerPage(ListAir list)
{
	DisplayTest(xKeyDisplay, ContentListAir, sizeof(ContentListAir) / sizeof(std::string));
...
void ListAirplaneFly(PNodeFli first, ListAir listAir)
{
	...
	SetColor(curColor);
	DisplayTest(xKeyDisplayFli, ContentAirplane, sizeof(ContentListAir) / sizeof(std::string));
...
void ChangePassFliMenuManagerPage(TreePass root, PNodeFli fli)
{
	DisplayTest(xKeyDisplayPassFli, ContentListPass, sizeof(ContentListPass) / sizeof(std::string));
